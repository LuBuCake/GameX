    ...
    public class Items
    {
        ...
        public static void WriteDefaultItems()
        {
            ...
            try
            {
                ...
                {
                    Serializer.WriteDataFile(@"GameX.Biohazard.5/prefab/item/" + $"{item.Portrait.ToLower()}.json", Serializer.SerializeItem(item));
                }
        ...
        public static List<Item> GetItemsFromFolder()
        {
            try
            {
                DirectoryInfo ItemsFolder = new DirectoryInfo(@"GameX.Biohazard.5/prefab/item/");
                FileInfo[] ItemFiles = ItemsFolder.GetFiles("*.json");
                List<Item> AvailableItems = new List<Item>();

                foreach (FileInfo ItemFile in ItemFiles)
                {
                    AvailableItems.Add(Serializer.DeserializeItem(File.ReadAllText(@"GameX.Biohazard.5/prefab/item/" + ItemFile.Name)));
                }
